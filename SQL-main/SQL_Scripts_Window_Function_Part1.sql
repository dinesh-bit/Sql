...
select e.*,
lag(salary, 0, 0) over(partition by dept_name order by emp_id) as prev_empl_sal,
...
select e.*,
lag(salary, 3, 0) over(partition by dept_name order by emp_id) as prev_empl_sal,
lead(salary, 3, 0) over(partition by dept_name order by emp_id) as next_empl_sal
from employee e;
